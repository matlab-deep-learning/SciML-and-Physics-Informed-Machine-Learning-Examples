...
% This example demonstrates how to use a *Neural Ordinary Differential Equation 
% (Neural ODE)* to model the dynamics of a nonlinear pendulum from noisy data. 
% The Neural ODE learns the time evolution of the pendulum's angle $\theta$ and 
% angular velocity $\dot{\theta}$ directly from observations.
% 
...
% 
% $$\frac{d}{\textrm{dt}}\left\lbrack \begin{array}{c}\theta \\\dot{\theta} 
% \end{array}\right\rbrack =\mathit{\mathbf{f}}\left(\theta ,\dot{\theta} \right)$$ 
% 
...
%% 
% Train on dataset where $t \leq 10$
...
% To model the right-hand side of the ODE, we define a neural network that approximates 
% the unknown dynamics function $f$. Here we use a multilayer perceptron (MLP) 
...
% fully connected layers>, each with 32 neurons
% * Activation function: <https://www.mathworks.com/help/deeplearning/ref/nnet.cnn.layer.gelulayer.html 
...
%% 
% Instead of training the model on the derivatives $\dot{\theta}$ and $\ddot{\theta}$, 
% we use an <https://www.mathworks.com/help/deeplearning/ref/dlarray.dlode45.html 
% ODE solver>, to integrate the output of fNet over time, starting from the initial 
% condition |Xtrain|. The solver produces a predicted trajectory of the state 
% $[\theta,\dot{\theta}]$, which is then compared to the observed data |Ytrain|, 
% and the network is optimized to minimize the error bewteen the predicted and 
% actual trajectories.  
% 
% The <https://www.mathworks.com/help/deeplearning/ref/nnet.cnn.layer.neuralodelayer.html 
% neuralODElayer> approximately solves the system
% 
% $\frac{d}{\textrm{dt}}\left\lbrack \begin{array}{c}\theta \\\dot{\theta} \end{array}\right\rbrack 
% =f\left(\theta ,\dot{\theta} \right)$, 
% 

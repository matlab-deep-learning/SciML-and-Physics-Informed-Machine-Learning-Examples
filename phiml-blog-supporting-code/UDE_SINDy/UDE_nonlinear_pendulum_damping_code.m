...
% This example demonstrates how to use a *Universal Differential Equation (UDE)* 
% to model the dynamics of a nonlinear pendulum from noisy observational data. 
% A UDE extends the concept of a neural ordinary differential equation (neural 
% ODE) by incorporating known physical laws alongside neural networks, enabling 
% the model to learn unknown or unmodeled dynamics.  
% 
% The true dynamics of a frictionless nonlinear pendulum are governed by the 
% second-order differential equation:
% 
% $\ddot{\theta} = -\omega_0^2\sin\theta$.
% 
% In this example, we assume the presense of an unknown frictional or damping 
% force, denoted by $h$:
% 
...
% 
% $\frac{d}{\textrm{dt}}\left\lbrack \begin{array}{c}\theta \\\dot{\theta} \end{array}\right\rbrack 
% =\left\lbrack \begin{array}{c}\dot{\theta} \\-\omega_0^2 \sin \;\theta +h\left(\theta 
% ,\dot{\;\theta } \right)\end{array}\right\rbrack =\mathit{\mathbf{g}}\left(\theta 
% ,\dot{\theta} \right)+\left\lbrack \begin{array}{c}0\\h\left(\theta ,\dot{\theta} 
% \right)\end{array}\right\rbrack$,
% 
...
%% 
% Use only the dataset where $t \leq 10$ to train.
...
% We define a function layer |gLayer| that encodes the known part of the dynamics, 
% $\mathbf{g}$. 
...
%% 
% We use a small multilayer perceptron (MLP) to approximate the unknown dynamics, 
% $h$:
...
%% 
% Construct the full network by connecting the input to both the known physics 
% and the neural network, and combining their outputs. 
...
%% 
% Use a <https://www.mathworks.com/help/deeplearning/ref/nnet.cnn.layer.neuralodelayer.html 
% |neuralODELayer|> to integrate the system over time, starting from the initial 
% condition.
...
%% 
% Denote $X = (\theta,\dot{\theta})$ and specify the basis functions $e_1(\theta,\dot{\theta}) 
% = \dot{\theta}$, $e_2(\theta,\dot{\theta}) = \dot{\theta}^2$, $e_3(\theta,\dot{\theta})=\dot{\theta}^3$. 
% Note that the dynamics $\mathbf{g}$ are not included as they are already known.
...
EEval = E(Y');
%% 
% Sequentially solve $h= WE$ for 10 iterations, and set terms with absolute 
% value less than 0.05 to 0.
...
% 
% True solution is: $h(\dot{\theta}) = -c_1\dot{\theta}- c_2\dot{\theta}^2$, 

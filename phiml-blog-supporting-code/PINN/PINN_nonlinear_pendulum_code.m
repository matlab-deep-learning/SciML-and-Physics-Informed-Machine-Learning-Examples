...
% This example demonstrates how to use a Physics-Informed Neural Networks (PINN) 
% to model the dynamics of a nonlinear pendulum governed by the second-order differential 
% equation:
% 
% $\ddot{\theta} = -\omega_0^2 \sin\theta$.
% 
% We assume access to sparse, noisy measurements of the pendulum's angular position 
% $\theta$ over a short time interval $[0,3]$, and aim to predict its behavior 
% over a longer interval $[0,10]$. Rather than relying solely on the data, the 
% PINN incorporates the governing ODE directly into the training process through 
% the loss function. 
% 
...
%% 
% Remove points from the training set to model the scenario where we have sparse 
% data. 
...
% * Hidden layers: 6 <https://www.mathworks.com/help/deeplearning/ref/nnet.cnn.layer.fullyconnectedlayer.html 
...
% To train the PINN, we define a composite loss function that combines physics, 
% data, and initial condition constraints. 
% 
% *Physics-Informed Loss*
% 
% The physics loss enforces the differential equation by penalizing deviations 
% from the governing pendulum ODE. Functions like <https://www.mathworks.com/help/releases/R2025a/deeplearning/ref/dlarray.dllaplacian.html?searchPort=60735 
% |dllaplacian|> and <https://www.mathworks.com/help/deeplearning/ref/dlarray.dljacobian.html 
% |dljacobian|> make it simple to define derivative terms appearing in ordinary 
% and partial differential equations for use in a PINN. 
...
%% 
% Alternatively, the physics loss can be created using the supporting function 
% |ode2PINNLossFunction| available for download at <https://www.mathworks.com/matlabcentral/fileexchange/172049-pinn-loss-function-generation-with-symbolic-math 
% PINN Loss Function Generation with Symbolic Math>.
% 
% *Initial Condition Loss*
% 
% The initial condition loss penalizes the network during training for violating 
% the initial condition $\theta(t=0)=\theta_0$.
...
% *Data Loss*
% 
% The data loss penalizes the difference between the network's predictions and 
% the given measurements of $\theta$.
...
%% 
% The total loss is a weighted combination of the physics, data, and initial 
% condition losses.  
...
% We will use the L-BFGS optimizer via the <https://www.mathworks.com/help/deeplearning/ref/lbfgsupdate.html 
% |lbfgsupdate|> function in a custom training loop in the next section.
% 
...
% Use the PINN to predict the value of $\theta$ at 100 evenly spaced points 
% between $[0,10]$. 
